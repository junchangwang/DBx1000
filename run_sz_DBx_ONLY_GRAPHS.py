...
def index_time_analysis(filename):
    ...
    for line in f:
        ...
        if len(a) != 5 and len(a) != 3:
            ...
            Svec.append(float(a[-1]))
            Svec_f.append(0)
        else:
            continue
    if len(Svec_f) != 0:
        ret.append(sum(Svec_f) / len(Svec_f)) 
    else:
        ret.append(0)
    if len(Svec) != 0:
